    ...
    {
        {
            std::shared_ptr<google::protobuf::io::ZeroCopyOutputStream> output(
                generator_context->OpenForInsert(filename_h_, std::string("class_scope:") + std::string(desc->full_name())));
    ...
    try
    {
        ...
        else if (dccl_field_options.units().has_unit())
        {
            ...
            construct_units_typedef_from_base_unit(
                field_name, dccl_field_options.units().unit(),
                dccl_field_options.units().relative_temperature(),
                dccl_field_options.units().prefix(), new_methods);
            construct_field_class_plugin(field_name, new_methods,
        ...
        else if (dccl_field_options.units().has_custom())
        {
            ...
            construct_units_typedef_from_custom_unit(
                field_name, dccl_field_options.units().custom().unit(),
                dccl_field_options.units().relative_temperature(),
                dccl_field_options.units().prefix(), new_methods);
            construct_field_class_plugin(field_name, new_methods,
                                         dccl::units::get_field_type_name(field->cpp_type()),
        ...
        else if (dccl_field_options.units().has_base_dimensions())
        {
                    ...
                    dimensions))
            {
                ...
                construct_base_dims_typedef(dimensions, powers, field_name, unit_system,
                                            dccl_field_options.units().relative_temperature(),
                                            dccl_field_options.units().prefix(), new_methods);
        ...
        else if (dccl_field_options.units().has_derived_dimensions())
        {
                    ...
                    dccl_field_options.units().derived_dimensions().end(), operators, dimensions))
            {
                ...
                construct_derived_dims_typedef(dimensions, operators, field_name, unit_system,
                                               dccl_field_options.units().relative_temperature(),
                                               dccl_field_options.units().prefix(), new_methods);
